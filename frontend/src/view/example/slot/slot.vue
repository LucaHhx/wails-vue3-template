...
export default {
  ...
  methods: {
      ...
      for (let i = 0; i < 10; i++) {
        for (let j = 0; j < 10; j++) {
          if (j === 0) {
            ...
            this.squareData[i][j] = chars[Math.floor(Math.random() * 7)]
          }
        }
      }
      this.score = 10
    ...
    clear(): void {
      ...
      while (true) {
        ...
        for (const square of del) {
          this.squareData[square[0]][square[1]] = '0'
        }
        for (let j: number = 0; j < n; ++j) {
          let t: number = m - 1
    ...
    dragStart(row: number, col: number): void {
      ...
      this.clickSquare[0] = row
      this.clickSquare[1] = col
    },
    dragEnd(event: any): void {
      ...
      if (Math.abs(x) >= Math.abs(y)) {
        if (x < 0 && col > 0) {
          ...
          this.squareData[row][col] = b
          this.squareData[row][col - 1] = a
        } else if (x > 0 && col < 9) {
          const a: string = this.squareData[row][col]
          const b: string = this.squareData[row][col + 1]
          this.squareData[row][col] = b
