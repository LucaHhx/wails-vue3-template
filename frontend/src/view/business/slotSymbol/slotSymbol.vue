  ...
  <div>
    <div class="gva-search-box">
      <el-form :inline="true" :model="searchInfo" class="demo-form-inline" @keyup.enter="onSubmit">
        ...
        <el-form-item label="单出是否赢钱">
          <el-select v-model="searchInfo.isSingleWin" :clearable="true" filterable placeholder="请选择">
            <el-option v-for="v,k in isSingleWinOpts" :key="k" :label="v" :value="parseInt(k)" />
          </el-select>
        </el-form-item>
    ...
    <div class="gva-table-box">
      ...
      >
        ...
        <el-table-column align="left" label="标签尺寸" prop="size" width="120" />
        <el-table-column align="left" label="标签层级" prop="hierarchy" width="120" />
        <el-table-column align="left" label="是否百搭" prop="isWild" width="120">
          <template #default="scope">
            <el-tag size="small" :type="statusTags[scope.row.isWild]" effect="dark">
              {{ isWildOpts[scope.row.isWild] }}
            </el-tag>
          </template>
        </el-table-column>
        <el-table-column align="left" label="替代标签" prop="sub" width="320" />
        <el-table-column sortable="custom" align="left" label="翻倍" prop="multiple" width="120" />
        <el-table-column align="left" label="单出是否赢钱" prop="isSingleWin" width="120">
          <template #default="scope">
            <el-tag size="small" :type="statusTags[scope.row.isSingleWin]" effect="dark">
              {{ isSingleWinOpts[scope.row.isSingleWin] }}
            </el-tag>
          </template>
        </el-table-column>
        ...
        <el-table-column align="left" label="按钮组">
          <template #default="scope">
            <el-button type="primary" link icon="edit" size="small" class="table-button" @click="updateSlotSymbolFunc(scope.row)">变更</el-button>
    ...
    <el-dialog v-model="dialogFormVisible" :before-close="closeDialog" title="弹窗操作">
      <el-form ref="elFormRef" :model="formData" label-position="right" :rules="rule" label-width="120px">
        ...
        <el-form-item label="是否百搭:" prop="isWild">
          <el-select v-model="formData.isWild" placeholder="请选择" style="width:100%" :clearable="true">
            <el-option v-for="v,k in isWildOpts" :key="k" :label="v" :value="parseInt(k)" />
          </el-select>
        </el-form-item>
        ...
        <el-form-item label="单出是否赢钱:" prop="isSingleWin">
          <el-select v-model="formData.isSingleWin" placeholder="请选择" style="width:100%" :clearable="true">
            <el-option v-for="v,k in isSingleWinOpts" :key="k" :label="v" :value="parseInt(k)" />
          </el-select>
        </el-form-item>
...
export default {
  ...
  data() {
    return {
      ...
      isWildOpts: { 1: '是', 2: '否' },
      isSingleWinOpts: { 1: '是', 2: '否' },
      statusOpts: { 1: '正常', 2: '关闭' },
    }
  }
}
</script>
...
const exportExcel = async() => {
  ...
  const table = await getSlotSymbolList({ page: 1, pageSize: 100000, ...searchInfo.value })
  if (table.code === 0) {
...
const getTableData = async() => {
  const table = await getSlotSymbolList({ page: page.value, pageSize: pageSize.value, ...searchInfo.value })
  if (table.code === 0) {
...
const onDelete = async() => {
  ...
  const res = await deleteSlotSymbolByIds({ ids })
  if (res.code === 0) {
    ElMessage({
      type: 'success',
      message: '删除成功'
...
// 更新行
const updateSlotSymbolFunc = async(row) => {
  const res = await findSlotSymbol({ ID: row.ID })
  type.value = 'update'
  if (res.code === 0) {
...
// 删除行
const deleteSlotSymbolFunc = async(row) => {
  const res = await deleteSlotSymbol({ ID: row.ID })
  if (res.code === 0) {
    ElMessage({
      type: 'success',
      message: '删除成功'
