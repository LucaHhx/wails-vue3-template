  ...
  <div>
    <div class="gva-search-box">
      <el-form :inline="true" :model="searchInfo" class="demo-form-inline" @keyup.enter="onSubmit">
        <el-form-item label="日期">
          <el-input v-model="searchInfo.date" placeholder="搜索条件" />
        </el-form-item>
    ...
    <div class="gva-table-box">
      ...
      >
        ...
        <el-table-column sortable="custom" align="left" label="次数" prop="num" width="120" />
        ...
        <el-table-column align="left" label="按钮组">
          <template #default="scope">
            <el-button type="primary" link icon="edit" size="small" class="table-button" @click="updateTrackingFunc(scope.row)">查看</el-button>
    ...
    <el-dialog v-model="dialogFormVisible" :before-close="closeDialog" title="弹窗操作">
      <el-form ref="elFormRef" :model="formData" label-position="right" :rules="rule" label-width="120px">
        <el-form-item label="日期:" prop="date">
          <el-input disabled  v-model="formData.date" :clearable="true" placeholder="请输入" />
        </el-form-item>
        <el-form-item label="类型:" prop="type">
          <el-select  disabled v-model="formData.type" placeholder="请选择" style="width:100%" :clearable="true">
            <el-option v-for="v,k in typeOpts" :key="k" :label="v" :value="parseInt(k)" />
          </el-select>
        </el-form-item>
        <el-form-item label="用户id:" prop="userId">
          <el-input disabled  v-model.number="formData.userId" :clearable="true" placeholder="请输入" />
        </el-form-item>
        <el-form-item label="次数:" prop="num">
          <el-input disabled v-model.number="formData.num" :clearable="true" placeholder="请输入" />
        </el-form-item>
      </el-form>
      <template #footer>
...
export default {
  ...
  data() {
    return {
      ...
      typeOpts: { 0: '登录', 1: '登录成功', 3: '加载资源', 4: '加载资源成功'},
    }
  }
}
</script>
...
const exportExcel = async() => {
  ...
  const table = await getTrackingList({ page: 1, pageSize: 100000, ...searchInfo.value })
  if (table.code === 0) {
...
const getTableData = async() => {
  const table = await getTrackingList({ page: page.value, pageSize: pageSize.value, ...searchInfo.value })
  if (table.code === 0) {
...
const onDelete = async() => {
  ...
  const res = await deleteTrackingByIds({ ids })
  if (res.code === 0) {
    ElMessage({
      type: 'success',
      message: '删除成功'
...
// 更新行
const updateTrackingFunc = async(row) => {
  const res = await findTracking({ ID: row.ID })
  type.value = 'update'
  if (res.code === 0) {
    formData.value = res.data.retracking
    dialogFormVisible.value = true
  }
}

// 删除行
const deleteTrackingFunc = async(row) => {
  const res = await deleteTracking({ ID: row.ID })
  if (res.code === 0) {
    ElMessage({
      type: 'success',
      message: '删除成功'
...
const enterDialog = async() => {
     elFormRef.value?.validate(async(valid) => {
       ...
       switch (type.value) {
         case 'create':
           res = await createTracking(formData.value)
           break
         case 'update':
           res = await updateTracking(formData.value)
           break
         default:
           res = await createTracking(formData.value)
           break
