  ...
  <div>
    ...
    <div class="gva-table-box">
      ...
      >
        ...
        <el-table-column align="left" label="机器特殊事件" prop="event1" width="250" />
        ...
        <el-table-column align="left" label="按钮组">
          <template #default="scope">
            <el-button type="primary" link icon="edit" size="small" class="table-button" @click="updateSlotEventFunc(scope.row)">变更</el-button>
    ...
    <el-dialog v-model="dialogFormVisible" :before-close="closeDialog" title="弹窗操作">
      <el-form ref="elFormRef" :model="formData" label-position="right" :rules="rule" label-width="120px">
        ...
        <el-form-item label="机器特殊事件:" prop="event1">
          <el-input v-model="formData.event1" :clearable="true" placeholder="请输入" />
        </el-form-item>
      </el-form>
      <template #footer>
...
const exportExcel = async() => {
  ...
  const table = await getSlotEventList({ page: 1, pageSize: 100000, ...searchInfo.value })
  if (table.code === 0) {
...
const getTableData = async() => {
  const table = await getSlotEventList({ page: page.value, pageSize: pageSize.value, ...searchInfo.value })
  if (table.code === 0) {
...
const onDelete = async() => {
  ...
  const res = await deleteSlotEventByIds({ ids })
  if (res.code === 0) {
    ElMessage({
      type: 'success',
      message: '删除成功'
...
// 更新行
const updateSlotEventFunc = async(row) => {
  const res = await findSlotEvent({ ID: row.ID })
  type.value = 'update'
  if (res.code === 0) {
    formData.value = res.data.reslotEvent
    dialogFormVisible.value = true
  }
}

// 删除行
const deleteSlotEventFunc = async(row) => {
  const res = await deleteSlotEvent({ ID: row.ID })
  if (res.code === 0) {
    ElMessage({
      type: 'success',
      message: '删除成功'
...
const enterDialog = async() => {
     elFormRef.value?.validate(async(valid) => {
       ...
       switch (type.value) {
         case 'create':
           res = await createSlotEvent(formData.value)
           break
         case 'update':
           res = await updateSlotEvent(formData.value)
           break
         default:
           res = await createSlotEvent(formData.value)
           break
