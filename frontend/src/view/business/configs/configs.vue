  ...
  <div>
    ...
    <div class="gva-table-box">
      ...
      >
        ...
        <el-table-column align="left" label="名称" prop="name" width="220" />
        <el-table-column align="left" label="值" prop="value" width="320" />
        ...
        <el-table-column align="left" label="按钮组">
          <template #default="scope">
            <el-button type="primary" link icon="edit" size="small" class="table-button" @click="updateConfigsFunc(scope.row)">变更</el-button>
...
const exportExcel = async() => {
  ...
  const table = await getConfigsList({ page: 1, pageSize: 100000, ...searchInfo.value })
  if (table.code === 0) {
...
const getTableData = async() => {
  const table = await getConfigsList({ page: page.value, pageSize: pageSize.value, ...searchInfo.value })
  if (table.code === 0) {
...
const onDelete = async() => {
  ...
  const res = await deleteConfigsByIds({ ids })
  if (res.code === 0) {
    ElMessage({
      type: 'success',
      message: '删除成功'
...
// 更新行
const updateConfigsFunc = async(row) => {
  const res = await findConfigs({ ID: row.ID })
  type.value = 'update'
  if (res.code === 0) {
    formData.value = res.data.reconfigs
    dialogFormVisible.value = true
  }
}

// 删除行
const deleteConfigsFunc = async(row) => {
  const res = await deleteConfigs({ ID: row.ID })
  if (res.code === 0) {
    ElMessage({
      type: 'success',
      message: '删除成功'
...
const enterDialog = async() => {
     elFormRef.value?.validate(async(valid) => {
       ...
       switch (type.value) {
         case 'create':
           res = await createConfigs(formData.value)
           break
         case 'update':
           res = await updateConfigs(formData.value)
           break
         default:
           res = await createConfigs(formData.value)
           break
