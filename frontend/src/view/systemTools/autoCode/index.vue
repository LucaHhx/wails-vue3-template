  ...
  <div>
    ...
    <div class="gva-search-box">
      ...
      <el-form ref="autoCodeForm" :rules="rules" :model="form" label-width="120px" :inline="true">
        ...
        <el-form-item>
          <template #label>
            <!-- <el-button size="small" type="info" @click="">去除前缀</el-button> -->
          </template>
        </el-form-item>
      </el-form>
    </div>
...
const setFdMap = async() => {
  const fdTypes = ['string', 'int', 'bool', 'float64', 'time.Time', 'tinyint']
...
const removePrefix = () => {
  // 如果前两个字母为大写
  if (isUpperCase(form.value.structName)) {
    form.value.structName = form.value.structName.slice(1)
    form.value.abbreviation = firstToLowerCase(form.value.abbreviation.slice(1))
    form.value.description = firstToLowerCase(form.value.description.slice(1))
    form.value.packageName = firstToLowerCase(form.value.packageName.slice(1))
  }
}
