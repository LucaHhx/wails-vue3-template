...
import {
  ...
  IngressesUpdate,
  IngressesDelete,
} from '@/plugin/kubernetes/api/ingress'
...
import { checkApi } from '@/plugin/kubernetes/utils/check'
const form = ref({})
const page = ref(1)
const total = ref(0)
...
const getTableData = async() => {
  const table = await IngressesList(searchInfo.value.cluster_id, page.value, pageSize.value, searchInfo.value.namespace, searchInfo.value.keywords, '', '')
  if (table.code === 0) {
...
const handleUpdate = async(row) => {
  type.value = 'update'
  title.value = '编辑'
  const res = await IngressesGet(searchInfo.value.cluster_id, row.metadata.namespace, row.metadata.name)
  if (res.code === 0) {
    form.value = res.data.items
    await CheckApi()
...
const handleDelete = async(row) => {
  row.visible = false
  const res = await IngressesDelete(searchInfo.value.cluster_id, row.metadata.namespace, row.metadata.name)
  if (res.code === 0) {
    ElMessage({
      type: 'success',
      message: '删除成功'
...
const enterDialog = async(row) => {
  ...
  switch (type.value) {
    case 'create':
      res = await IngressesCreate(searchInfo.value.cluster_id, row.metadata.namespace, row)
      break
    case 'update':
      res = await IngressesUpdate(searchInfo.value.cluster_id, row.metadata.namespace, row.metadata.name, row)
      break
    default:
      res = await IngressesCreate(searchInfo.value.cluster_id, row)
      break
  }
...
const openDialog = async() => {
  ...
  title.value = '创建'
  await CheckApi()
  form.value = getK8sObject('ingresses', searchInfo.value.namespace, NewVersion.value)
...
const openAddDialog = async() => {
  ...
  readOnly.value = false
  await CheckApi()
  form.value = getK8sObject('ingressesinit', searchInfo.value.namespace, NewVersion.value)
  dialogAddVisible.value = true
}
