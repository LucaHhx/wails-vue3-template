...
import {
  ...
  DeploymentsCreate,
  DeploymentsDelete,
} from '@/plugin/kubernetes/api/deployments'
...
const getTableData = async() => {
  const table = await DeploymentsList(searchInfo.value.cluster_id, page.value, pageSize.value, searchInfo.value.namespace, searchInfo.value.keywords)
  if (table.code === 0) {
...
const handleUpdate = async(row) => {
  ...
  title.value = '编辑'
  readOnly.value = true
  const res = await DeploymentsGet(searchInfo.value.cluster_id, row.metadata.namespace, row.metadata.name)
  if (res.code === 0) {
...
const handleDelete = async(row) => {
  row.visible = false
  const res = await DeploymentsDelete(searchInfo.value.cluster_id, row.metadata.namespace, row.metadata.name)
  if (res.code === 0) {
    ElMessage({
      type: 'success',
      message: '删除成功'
...
const enterDialog = async(row) => {
  ...
  switch (type.value) {
    case 'create':
      res = await DeploymentsCreate(searchInfo.value.cluster_id, row.metadata.namespace, row)
      break
    case 'update':
      res = await DeploymentsUpdate(searchInfo.value.cluster_id, row.metadata.namespace, row.metadata.name, row)
      break
    default:
      res = await DeploymentsCreate(searchInfo.value.cluster_id, row)
      break
  }
