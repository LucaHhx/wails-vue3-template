  ...
  <div>
    <div class="gva-table-box">
      <div class="gva-btn-list">
        ...
        <el-button size="small" type="primary" icon="plus" @click="openAddDialog('addApi')">新增</el-button>      </div>
      <div v-if="tableData">
...
import {
  ...
  JobsCreate,
  JobsDelete,
} from '@/plugin/kubernetes/api/jobs'
...
import CreateBlock from '@/plugin/kubernetes/components/create/index.vue'
...
const getTableData = async() => {
  const table = await JobsList(searchInfo.value.cluster_id, page.value, pageSize.value, searchInfo.value.namespace, searchInfo.value.keywords)
  if (table.code === 0) {
...
const handleUpdateYAML = async(row) => {
  type.value = 'update'
  title.value = '编辑'
  const res = await JobsGet(searchInfo.value.cluster_id, row.metadata.namespace, row.metadata.name)
  if (res.code === 0) {
...
const handleDelete = async(row) => {
  row.visible = false
  const res = await JobsDelete(searchInfo.value.cluster_id, row.metadata.namespace, row.metadata.name)
  if (res.code === 0) {
    ElMessage({
      type: 'success',
      message: '删除成功'
...
const enterDialog = async(row) => {
  ...
  switch (type.value) {
    case 'create':
      res = await JobsCreate(searchInfo.value.cluster_id, row.metadata.namespace, row)
      break
    case 'update':
      res = await JobsUpdate(searchInfo.value.cluster_id, row.metadata.namespace, row.metadata.name, row)
      break
    default:
      res = await JobsCreate(searchInfo.value.cluster_id, row)
      break
  }
