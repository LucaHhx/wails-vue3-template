<template>
  <div>
    <el-table ref="multipleTable" :data="tableData" style="width: 100%" tooltip-effect="dark" row-key="ID">
      ...
      >
        <template #default="scope">
          <div v-show="scope.row.metadata.labels">
            <span v-for="(k, v, index) in scope.row.metadata.labels" :key="index" class="span-color-tag">
              <span type="info" @click="handleUpdate(scope.row)"> {{ v }}: {{ k }}
              </span>
            </span>
          </div>
          <div v-show="!scope.row.metadata.labels">
