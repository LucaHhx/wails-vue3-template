<template>
  <div>
    <el-table ref="multipleTable" :data="tableData" style="width: 100%" tooltip-effect="dark" row-key="ID">
      ...
      <el-table-column label="期望的Pod(个)" prop="status.desired" type="scope">
        <template #default="scope">
          <span v-if="scope.row.status.desired"> {{ scope.row.status.desired }}</span>
          <el-tooltip placement="top">
            <template #content>
              <span>等同于当前集群中匹配的 node 数量</span>
            </template>
            <el-button size="small" type="primary" link icon="WarningFilled" />
          </el-tooltip>
        </template>
      </el-table-column>
      <el-table-column label="运行中的Pod(个)" prop="status.active" />
      <el-table-column label="执行成功的Pod(个)" prop="status.succeeded" />
      <el-table-column label="执行失败的Pod(个)" prop="status.failed" />
      <el-table-column
        label="阶段"
        prop="phase"
        type="scope"
      >
        <template #default="scope">
          ...
          <el-tag v-else-if="scope.row.status.phase ==='completed'" type="info">Completed</el-tag>
