...
import {
  ...
  CloneSetsCreate,
  CloneSetsDelete,
} from '@/plugin/kubernetes/api/openkruise/clonesets'
...
const getTableData = async() => {
  const table = await CloneSetsList(searchInfo.value.cluster_id, page.value, pageSize.value, searchInfo.value.namespace, searchInfo.value.keywords, '')
  if (table.code === 0) {
...
const handleUpdate = async(row) => {
  type.value = 'update'
  title.value = '编辑'
  const res = await CloneSetsGet(searchInfo.value.cluster_id, row.metadata.namespace, row.metadata.name)
  if (res.code === 0) {
...
const handleDelete = async(row) => {
  row.visible = false
  const res = await CloneSetsDelete(searchInfo.value.cluster_id, row.metadata.namespace, row.metadata.name)
  if (res.code === 0) {
    ElMessage({
      type: 'success',
      message: '删除成功'
...
const enterDialog = async(row) => {
  ...
  switch (type.value) {
    case 'create':
      res = await CloneSetsCreate(searchInfo.value.cluster_id, row.metadata.namespace, row)
      break
    case 'update':
      res = await CloneSetsUpdate(searchInfo.value.cluster_id, row.metadata.namespace, row.metadata.name, row)
      break
    default:
      res = await CloneSetsCreate(searchInfo.value.cluster_id, row)
      break
  }
...
const openDialog = () => {
  type.value = 'create'
  title.value = '创建'
  form.value = getK8sObject('cloneset', searchInfo.value.namespace, '')
  dialogYamlVisible.value = true
}
...
const handleDetail = async(row) => {
  ...
  if (res.code === 0) {
    ...
    form.value['type'] = 'advancedscloneset'
