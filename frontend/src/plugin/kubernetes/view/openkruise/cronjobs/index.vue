...
import {
  ...
  CronJobsCreate,
  CronJobsDelete,
} from '@/plugin/kubernetes/api/openkruise/cronjobs'
...
const getTableData = async() => {
  const table = await CronJobsList(searchInfo.value.cluster_id, page.value, pageSize.value, searchInfo.value.namespace, searchInfo.value.keywords, '')
  if (table.code === 0) {
...
const handleUpdate = async(row) => {
  type.value = 'update'
  title.value = '编辑'
  const res = await CronJobsGet(searchInfo.value.cluster_id, row.metadata.namespace, row.metadata.name)
  if (res.code === 0) {
...
const handleDelete = async(row) => {
  row.visible = false
  const res = await CronJobsDelete(searchInfo.value.cluster_id, row.metadata.namespace, row.metadata.name)
  if (res.code === 0) {
    ElMessage({
      type: 'success',
      message: '删除成功'
...
const enterDialog = async(row) => {
  ...
  switch (type.value) {
    case 'create':
      res = await CronJobsCreate(searchInfo.value.cluster_id, row.metadata.namespace, row)
      break
    case 'update':
      res = await CronJobsUpdate(searchInfo.value.cluster_id, row.metadata.namespace, row.metadata.name, row)
      break
    default:
      res = await CronJobsCreate(searchInfo.value.cluster_id, row)
      break
  }
...
const openDialog = () => {
  type.value = 'create'
  title.value = '创建'
  form.value = getK8sObject('advancedcronjob', searchInfo.value.namespace, '')
  dialogYamlVisible.value = true
}
...
const handleDetail = async(row) => {
  ...
  if (res.code === 0) {
    ...
    form.value['type'] = 'advancedcronjob'
