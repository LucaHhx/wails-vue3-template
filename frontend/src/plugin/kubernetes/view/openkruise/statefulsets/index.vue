...
import {
  ...
  StatefulsetsCreate,
  StatefulsetsDelete,
} from '@/plugin/kubernetes/api/openkruise/statefulsets'
...
const openDialog = () => {
  type.value = 'create'
  title.value = '创建'
  form.value = getK8sObject('advancedstatefulset', searchInfo.value.namespace, '')
  dialogYamlVisible.value = true
}
...
const handleDetail = async(row) => {
  ...
  if (res.code === 0) {
    ...
    form.value['type'] = 'advancedstatefulset'
