...
import {
  ...
  SidecarSetsCreate,
  SidecarSetsDelete,
} from '@/plugin/kubernetes/api/openkruise/sidecarsets'
...
const getTableData = async() => {
  const table = await SidecarSetsList(searchInfo.value.cluster_id, page.value, pageSize.value, searchInfo.value.keywords, '')
  if (table.code === 0) {
...
const handleUpdate = async(row) => {
  type.value = 'update'
  title.value = '编辑'
  const res = await SidecarSetsGet(searchInfo.value.cluster_id, row.metadata.name)
  if (res.code === 0) {
...
const handleDelete = async(row) => {
  row.visible = false
  const res = await SidecarSetsDelete(searchInfo.value.cluster_id, row.metadata.name)
  if (res.code === 0) {
    ElMessage({
      type: 'success',
      message: '删除成功'
...
const enterDialog = async(row) => {
  ...
  switch (type.value) {
    case 'create':
      res = await SidecarSetsCreate(searchInfo.value.cluster_id, row)
      break
    case 'update':
      res = await SidecarSetsUpdate(searchInfo.value.cluster_id, row.metadata.name, row)
      break
    default:
      res = await SidecarSetsCreate(searchInfo.value.cluster_id, row)
      break
...
const openDialog = () => {
  type.value = 'create'
  title.value = '创建'
  form.value = getK8sObject('sidecarsets', '')
  dialogYamlVisible.value = true
}
