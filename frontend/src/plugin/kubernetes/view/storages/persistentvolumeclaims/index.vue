...
} from '@/plugin/kubernetes/api/persistentvolumeclaims'
...
const getTableData = async() => {
  const table = await PersistentVolumeClaimsList(searchInfo.value.cluster_id, page.value, pageSize.value, searchInfo.value.namespace, searchInfo.value.keywords, '')
  if (table.code === 0) {
...
const handleUpdate = async(row) => {
  type.value = 'update'
  title.value = '编辑'
  const res = await PersistentVolumeClaimsGet(searchInfo.value.cluster_id, row.metadata.namespace, row.metadata.name)
  if (res.code === 0) {
...
const handleDelete = async(row) => {
  row.visible = false
  const res = await PersistentVolumeClaimsDelete(searchInfo.value.cluster_id, row.metadata.namespace, row.metadata.name)
  if (res.code === 0) {
    ElMessage({
      type: 'success',
      message: '删除成功'
...
const enterDialog = async(row) => {
  ...
  switch (type.value) {
    case 'create':
      res = await PersistentVolumeClaimsCreate(searchInfo.value.cluster_id, row.metadata.namespace, row)
      break
    case 'update':
      res = await PersistentVolumeClaimsUpdate(searchInfo.value.cluster_id, row.metadata.namespace, row.metadata.name, row)
      break
    default:
      res = await PersistentVolumeClaimsCreate(searchInfo.value.cluster_id, row)
      break
  }
