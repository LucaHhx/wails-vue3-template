...
} from '@/plugin/kubernetes/api/poddisruptionbudgets'
...
const getTableData = async() => {
  const table = await PodDisruptionbudgetsList(searchInfo.value.cluster_id, page.value, pageSize.value, searchInfo.value.namespace, searchInfo.value.keywords, '')
  if (table.code === 0) {
...
const handleUpdate = async(row) => {
  type.value = 'update'
  title.value = '编辑'
  const res = await PodDisruptionbudgetsGet(searchInfo.value.cluster_id, row.metadata.namespace, row.metadata.name)
  if (res.code === 0) {
...
const handleDelete = async(row) => {
  row.visible = false
  const res = await PodDisruptionbudgetsDelete(searchInfo.value.cluster_id, row.metadata.namespace, row.metadata.name)
  if (res.code === 0) {
    ElMessage({
      type: 'success',
      message: '删除成功'
...
const enterDialog = async(row) => {
  ...
  switch (type.value) {
    case 'create':
      res = await PodDisruptionbudgetsCreate(searchInfo.value.cluster_id, row.metadata.namespace, row)
      break
    case 'update':
      res = await PodDisruptionbudgetsUpdate(searchInfo.value.cluster_id, row.metadata.namespace, row.metadata.name, row)
      break
    default:
      res = await PodDisruptionbudgetsCreate(searchInfo.value.cluster_id, row)
      break
  }
