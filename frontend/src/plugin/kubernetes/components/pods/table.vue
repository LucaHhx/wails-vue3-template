...
const PodsListData = async() => {
  const table = await PodsList(cluster_id.value, page.value, pageSize.value, '', '', '', `spec.nodeName=${keywords.value}`)
  if (table.code === 0) {
...
const handleDelete = async(row) => {
  ElMessageBox.confirm('此操作将删除Pod: ' + row.metadata.name + ' 是否继续?',
    ...
    const res = await PodsDelete(cluster_id.value, row.metadata.namespace, row.metadata.name)
    if (res.code === 0) {
      ElMessage({
        type: 'success',
        message: '删除成功',
...
const handleDetail = async(row) => {
  title.value = '详情'
  podDetail.value = row
  const res = await EventsList(cluster_id.value, row.metadata.name, row.metadata.namespace)
  if (res.code === 0) {
...
const handleEdit = async(row) => {
  title.value = '编辑'
  const res = await PodsGet(cluster_id.value, row.metadata.namespace, row.metadata.name)
  if (res.code === 0) {
...
const enterDialog = async(row) => {
  const res = await PodsUpdate(cluster_id.value, row.metadata.namespace, row.metadata.name, row)
  if (res.code === 0) {
    ElMessage({
      type: 'success',
      message: '更新成功!'
