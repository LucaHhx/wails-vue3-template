...
import {
  ReplicaSetsList,
} from '@/plugin/kubernetes/api/replicasets'
import { formatTimeToStr } from '@/plugin/kubernetes/utils/date'
import YamlBlock from '@/plugin/kubernetes/components/yaml/index.vue'
import { ElMessage, ElMessageBox } from 'element-plus'
import { DeploymentsPatch } from '@/plugin/kubernetes/api/deployments'
...
const DeploymentreplicasInit = async() => {
  const res = await ReplicaSetsList(Detaildata.value.cluster_id, Detaildata.value.metadata.namespace, Selectors.value)
...
const handleRollback = (row) => {
  ElMessageBox.confirm('此操作将回滚当前 deployment 到改版本，是否继续?',
    ...
    const res = await DeploymentsPatch(Detaildata.value.cluster_id, row.metadata.namespace, Detaildata.value.metadata.name, { spec: { template: row.spec.template }})
    if (res.code === 0) {
      ElMessage({
        type: 'success',
        message: '回滚成功',
