import service from '@/utils/request'

// @Tags User
// @Summary 创建User
// @Security ApiKeyAuth
// @accept application/json
// @Produce application/json
// @Param data body model.User true "创建User"
// @Success 200 {string} string "{"success":true,"data":{},"msg":"获取成功"}"
// @Router /bUser/createUser [post]
export const createUser = (data) => {
  return service({
    url: '/bUser/createUser',
    method: 'post',
    data
  })
}

// @Tags User
// @Summary 删除User
// @Security ApiKeyAuth
// @accept application/json
// @Produce application/json
// @Param data body model.User true "删除User"
// @Success 200 {string} string "{"success":true,"data":{},"msg":"删除成功"}"
// @Router /bUser/deleteUser [delete]
export const deleteUser = (data) => {
  return service({
    url: '/bUser/deleteUser',
    method: 'delete',
    data
  })
}

// @Tags User
// @Summary 删除User
// @Security ApiKeyAuth
// @accept application/json
// @Produce application/json
// @Param data body request.IdsReq true "批量删除User"
// @Success 200 {string} string "{"success":true,"data":{},"msg":"删除成功"}"
// @Router /bUser/deleteUser [delete]
export const deleteUserByIds = (data) => {
  return service({
    url: '/bUser/deleteUserByIds',
    method: 'delete',
    data
  })
}

// @Tags User
// @Summary 更新User
// @Security ApiKeyAuth
// @accept application/json
// @Produce application/json
// @Param data body model.User true "更新User"
// @Success 200 {string} string "{"success":true,"data":{},"msg":"更新成功"}"
// @Router /bUser/updateUser [put]
export const updateUser = (data) => {
  return service({
    url: '/bUser/updateUser',
    method: 'put',
    data
  })
}

export const changeUserPassword = (data) => {
  return service({
    url: '/bUser/changePassword',
    method: 'put',
    data
  })
}

export const changeAmount = (data) => {
  return service({
    url: '/bUser/changeAmount',
    method: 'put',
    data
  })
}

// @Tags User
// @Summary 用id查询User
// @Security ApiKeyAuth
// @accept application/json
// @Produce application/json
// @Param data query model.User true "用id查询User"
// @Success 200 {string} string "{"success":true,"data":{},"msg":"查询成功"}"
// @Router /bUser/findUser [get]
export const findUser = (params) => {
  return service({
    url: '/bUser/findUser',
    method: 'get',
    params
  })
}

// @Tags User
// @Summary 分页获取User列表
// @Security ApiKeyAuth
// @accept application/json
// @Produce application/json
// @Param data query request.PageInfo true "分页获取User列表"
// @Success 200 {string} string "{"success":true,"data":{},"msg":"获取成功"}"
// @Router /bUser/getUserList [get]
export const getUserList = (params) => {
  return service({
    url: '/bUser/getUserList',
    method: 'get',
    params
  })
}
